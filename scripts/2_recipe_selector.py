...
Usage:
  python scripts/2_recipe_selector.py
    -> auto-selects the simplest recipe (current behavior)

  python scripts/2_recipe_selector.py P01_R01
    -> selects the specified recipe ID
"""
...
def select_recipe(recipes, recipe_timestamps, narrations, requested_recipe_id=None):
    """
    Select recipe by optional ID, otherwise auto-select simplest recipe.
    """
    ...
    # Select either requested recipe or auto-select simplest one.
    if requested_recipe_id:
        requested_recipe_id = requested_recipe_id.strip()
        selected_matches = recipe_df[recipe_df["recipe_id"] == requested_recipe_id]

        if selected_matches.empty:
            available_examples = ", ".join(recipe_df["recipe_id"].head(20).tolist())
            raise ValueError(
                f"Requested recipe_id '{requested_recipe_id}' was not found (or has no videos). "
                f"Example available IDs: {available_examples}"
            )

        selected_row = selected_matches.iloc[0]
        selected_recipe_id = requested_recipe_id
    else:
        selected_row = recipe_df.iloc[0]  # Current default behavior
...
def main():
    parser = argparse.ArgumentParser(
        description="Select recipe data for downstream graph generation."
    )
    parser.add_argument(
        "recipe_id",
        nargs="?",
        default=None,
        help="Optional recipe ID to select directly (example: P01_R01).",
    )
